package tpv.model.dao.base.hibernate.spring.impl;

import tpv.model.dao.base.hibernate.GenericHibernateDAO;
import tpv.model.vo.base.CreatedUpdatedEntity;
...
public abstract class GenericHibernateSpringDAOImpl<T, ID extends Serializable> extends HibernateDaoSupport implements GenericHibernateDAO<T, ID> {
    ...
    public T makePersistent(T entity) throws DataAccessException {
        if(entity instanceof CreatedUpdatedEntity) {
            ((CreatedUpdatedEntity)entity).setUpdated(new Date()); //TODO: Dirty checking

            if(((CreatedUpdatedEntity)entity).getCreated() == null) {
                ((CreatedUpdatedEntity)entity).setCreated(new Date());
            }
        }
